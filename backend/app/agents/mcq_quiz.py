...
def related_chunks(topic: str) -> list[str]:
...
def generate_complete_summary(chunks: list[str]) -> str:
    combined_chunks = "\n\n".join(chunks)
...
def generate_mcq_quiz(
    num_questions: int, summary: str, difficulty: str
) -> list[MCQQuestion]:
    ...
    for topic in topics:
        try:
            chunks = related_chunks(topic)

            if not chunks:
                continue

            complete_summary = generate_complete_summary(chunks)

            if not complete_summary:
                continue

            question = generate_mcq_question(complete_summary, difficulty)
