...
function AttemptPublicQuizPage() {
  ...
  const initSession = useCallback(async () => {
    ...
    setError(null)
    try {
      const sessionRes = await authFetch(`/api/v1/public_quizzes/${uuid}/sessions`, {
        method: 'POST',
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center space-y-4 max-w-sm">
          ...
          <button
            onClick={() => navigate({ to: '/dashboard/public-quizzes' })}
            className="px-6 py-3 bg-violet-600 text-white rounded-2xl font-black text-xs uppercase tracking-wider"
          >
            Back to Explore
          </button>
        </div>
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="max-w-4xl mx-auto space-y-8 pt-4 pb-20">
      ...
      <motion.div variants={item} className="bg-white/50 backdrop-blur-md border border-slate-200/60 rounded-[2rem] p-6 shadow-sm flex items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-violet-500 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-violet-200">
            <Sparkles className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-lg font-black text-slate-800 leading-none mb-1">Public Quiz</h2>
        ...
        <div className="flex-1 max-w-xs">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[10px] font-black text-violet-600 uppercase tracking-widest">{currentIdx + 1}/{total}</span>
          </div>
          <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
            <motion.div animate={{ width: `${progress}%` }} className="h-full bg-violet-500 rounded-full transition-all duration-500" />
          </div>
        </div>
      ...
      {currentQuestion && (
        <motion.div
          ...
          className="bg-white border border-slate-200/60 rounded-[2.5rem] p-10 shadow-xl shadow-violet-500/5"
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-50 text-violet-600 text-[10px] font-black uppercase tracking-widest mb-6">
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-between p-5 rounded-2xl border transition-all text-left disabled:cursor-default ${isSelected
                    ...
                    : 'border-slate-100 bg-slate-50/50 hover:border-violet-300 hover:bg-violet-50/30 disabled:hover:border-slate-100 disabled:hover:bg-slate-50/50'
                    }`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-black transition-colors border ${isSelected ? 'bg-violet-500 border-violet-500 text-white' : 'bg-white border-slate-200 text-slate-400'
                      }`}>
                      {key}
                    </div>
        ...
        <div className="flex gap-4">
          ...
          {!showFeedback ? (
            <button
              ...
              className="flex items-center gap-3 px-10 py-4 bg-slate-900 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-violet-500 transition-all shadow-xl shadow-slate-900/10 active:scale-95 group"
            >
            ...
            <button
              onClick={handleNext}
              disabled={streaming}
              className="flex items-center gap-3 px-10 py-4 bg-violet-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-violet-600 transition-all shadow-xl active:scale-95 group"
            >
