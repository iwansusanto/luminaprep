...
import { ChevronRight, Clock, Info, CheckCircle2, ChevronLeft, Sparkles, Loader2, AlertCircle } from 'lucide-react'
...
function StartQuizPage() {
  ...
  // Start session and load questions
  const initSession = useCallback(async () => {
    ...
    try {
      ...
      const qRes = await fetch(`/api/v1/quiz_sessions/${session.id}/questions`, {
        credentials: 'include',
      })
  ...
  const handleSubmitAnswer = async () => {
    ...
    setSubmitting(true)
    try {
      await fetch(`/api/v1/quiz_sessions/${sessionData.session_id}/submit_answer`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
          question_id: currentQuestion.id,
          // Send the option value, not the key
  ...
  const handleNext = async () => {
    ...
    if (currentIdx < sessionData.total_questions - 1) {
      ...
      // Complete session
      try {
        await fetch(`/api/v1/quiz_sessions/${sessionData.session_id}/complete`, {
          method: 'POST',
          credentials: 'include',
        })
      } catch { /* ignore */ }
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="max-w-4xl mx-auto space-y-8 pt-4 pb-20">
      ...
      {currentQuestion && (
        <motion.div
          ...
          className="bg-white border border-slate-200/60 rounded-[2.5rem] p-10 shadow-xl shadow-emerald-500/5 relative overflow-hidden"
        >
                  ...
                  <button
                    ...
                    className={`flex items-center justify-between p-5 rounded-2xl border transition-all text-left ${
                      isSelected
                        ...
                        : 'border-slate-100 bg-slate-50/50 hover:border-emerald-300 hover:bg-emerald-50/30'
                    }`}
                  >
                    <div className="flex items-center gap-4">
