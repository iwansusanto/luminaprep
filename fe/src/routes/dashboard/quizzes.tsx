...
function QuizzesPage() {
  const data = useMemo<Quiz[]>(() => [
    { id: 1, name: 'Organic Chemistry Basics', score: 85, date: '2h ago', complexity: 'Intermediate', questions: 20, status: 'Completed' },
    { id: 2, name: 'Linear Algebra: Matrices', score: 92, date: 'Yesterday', complexity: 'Mastery', questions: 15, status: 'Completed' },
    { id: 3, name: 'European History 1800s', score: null, date: 'Ready to start', complexity: 'Beginner', questions: 30, status: 'Ready' },
    { id: 4, name: 'Quantum Physics Intro', score: 78, date: '3 days ago', complexity: 'Mastery', questions: 10, status: 'Completed' },
    { id: 5, name: 'Microeconomics Principles', score: null, date: 'Draft', complexity: 'Intermediate', questions: 25, status: 'Draft' },
  ], [])
    ...
    columnHelper.display({
      ...
      cell: info => (
        <div className="flex items-center justify-end gap-2">
          {info.row.original.status === 'Ready' || info.row.original.status === 'Completed' ? (
            <button className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-md shadow-indigo-100 active:scale-95">
              <Play className="w-3 h-3 fill-current" />
              {info.row.original.status === 'Completed' ? 'Retake' : 'Start'}
            </button>
          ) : (
            <button className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
              Continue
            </button>
          )}
